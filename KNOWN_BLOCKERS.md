...
- Full login success is blocked on production account/default account loading, remaining `sendLoginClient` branches, `sendLoginRC`/`sendLoginNC`, and world warp behavior.
...
- `CFileQueue` queueing/compression/socket flushing is documented but not implemented in C# yet.
- Production account loading is blocked on exact filesystem/default-account save behavior and C++ member defaults.
- World-entry/warp is traced, but not implemented because it immediately enters level/map/NPC/resource runtime.
