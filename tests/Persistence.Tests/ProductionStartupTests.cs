...
public sealed class ProductionStartupTests
{
    ...
    {
        var parsed = ProductionStartupCommandLine.Parse(
    ...
    public void ParseEnvironmentOnlyWhenUseEnvExistsAndServerGatesOtherOverrides()
    {
        ...
        var parsed = ProductionStartupCommandLine.Parse(["gserver", "--server", "ignored"], env.GetValueOrDefault);
    ...
    public void ResolveUsesStartupServerBeforeDirectorySearch()
    {
        ...
        var result = ProductionStartupResolver.Resolve(root, ProductionStartupOverrides.Empty);

        Assert.True(result.Success);
        Assert.Equal("from-file", result.ServerName);
        Assert.Equal(ProductionStartupServerSource.StartupServerFile, result.Source);
    }
    ...
    public void ResolveUsesOnlyServerDirectoryWhenStartupFileIsEmpty()
    {
        ...
        Assert.Equal(ProductionStartupServerSource.SingleServersDirectory, result.Source);
    ...
    public void ResolveFailsWhenNoOverrideStartupFileOrSingleDirectoryExists()
    {
        ...
        Assert.Equal(ProductionStartupServerSource.None, result.Source);
    }
}
