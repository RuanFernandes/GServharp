...
public sealed class ProductionTimingBoundaryTests
{
    [Fact]
    public void DoMainAlwaysSchedulesSocketUpdateBeforeOneSecondTimedEvents()
    {
        var scheduler = new ProductionTimingScheduler(TimeSpan.Zero);
    ...
    public void DoMainDoesNotRunOneSecondTimedEventsBeforeOneSecondElapsed()
    {
        ...
        Assert.Equal(new[] { ProductionTimingAction.SocketManagerUpdate5ms }, actions);
    }
    ...
    public void PeriodicServerJobsPreserveCppOrderAtFiveMinutes()
    {
        ...
        Assert.Equal(
            new[]
            {
                ...
                ProductionTimingAction.LevelTimedEvents,
                ProductionTimingAction.GroupLevelTimedEvents,
                ProductionTimingAction.CalculateServerTime,
                ProductionTimingAction.BroadcastNewWorldTime,
                ProductionTimingAction.SaveServerFlags,
                ProductionTimingAction.ResyncFileSystemAccounts,
                ProductionTimingAction.ResyncFileSystems,
                ProductionTimingAction.LoadAllowedVersions,
                ProductionTimingAction.LoadServerMessage,
                ProductionTimingAction.LoadIpBans,
                ProductionTimingAction.SaveWeapons,
                ProductionTimingAction.CleanupEmptyInstancedGroupLevels
            },
            actions);
    }
    ...
    public void NewWorldTimeCalculationAndPacketMatchConfirmedCppEncoding()
    {
        ...
        var packet = ProductionTimingPackets.BuildNewWorldTime(time);
