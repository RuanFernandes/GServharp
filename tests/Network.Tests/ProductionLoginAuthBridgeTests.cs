...
public sealed class ProductionLoginAuthBridgeTests
{
    [Fact]
    public void BeginClientLoginSendsVerifyAccountAndWaitsForServerListResponse()
    {
        ...
        var result = bridge.BeginClientLogin(new ProductionSocketSessionContext(7, "127.0.0.1"), Client3LoginPacket());
    ...
    public void HandleVerifyAccount2ResponseReturnsDisconnectBytesForRejectedLogin()
    {
        var gateway = new RecordingGateway { IsConnected = true };
        var bridge = new ProductionLoginAuthBridge(gateway, AuthOptions());
        _ = bridge.BeginClientLogin(new ProductionSocketSessionContext(7, "127.0.0.1"), Client3LoginPacket());
